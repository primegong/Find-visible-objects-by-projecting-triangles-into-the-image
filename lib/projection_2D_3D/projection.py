...
def get_instance_id(bbox, masks, tria_2D, tria_area, mask_area = None):
	...
	for idx, key in enumerate(masks.keys()):
		...
		if delta_x> 0  and delta_y>0:
			...
			# overlap = polygon_IOU(tria_2D, contours, tria_area, mask_area)
			...
			continue
	return instance_id, max_overlap
...
def is_fg_in_image(tria_2D, masks):
	...
	tria_area = count_pixels_in_tria_2D(bbox, tria_2D)
	# # count the area
	# tria_area = cal_area(tria_2D)
	tria_area = np.maximum(tria_area, 0.001)

	if tria_area > 0:
		#当三角形内部像素数不为0时，求与该三角形交集最大的instance mask，以及交集部分前景像素数量；
		# get the corresponding instance_id
		_instance_id, max_overlap = get_instance_id(bbox, masks, tria_2D, tria_area)
		# threshold:0.5
		if max_overlap / tria_area >= 0.5:
		...
		for idx, key in enumerate(masks.keys()):
			...
			if ct == 3:
				...
				continue
	return instance_id, bbox, tria_area
...
def project_to_subregion(j, triangle, segment, args, K, distortion, height, width, stride, region_size, x_num_region, total_masks, relation, obj_files):
	...
	instance_id, bbox, tria_area = is_fg_in_image(tria_2D, masks)
	...
	if instance_id != -1:
		...
		if not key in relation.keys():
			# 该目标无已有三角形，直接保存
			#id 为三角形索引
			relation[key] = {'id': [], 'normal': [], 'tria_2D': [], 'bbox': [], 'distance': [], 'tria_area': []}
			flag = True
		else:
			# # no compare:
			#  flag = True
			...
			info = relation[key]
			flag, bg_indexes = is_fg_in_3Dspace(info, tria_2D, distance, bbox, tria_area)
			...
			if len(bg_indexes):
				...
				for ind in bg_indexes:
					...
					del (relation[key]['tria_area'][ind])
		...
		if flag:
			...
			relation[key]['tria_area'].append(tria_area)
...
def compute_overlap(info, tria_2D, bbox, tria_area, threshold = 0.5):
	...
	if len(overlap_inds):
		...
		areas_of_trias = np.array(info['tria_area'])[overlap_inds]
		areas_of_trias = np.maximum(areas_of_trias, 0.001)
		...
		if tria_area ==0:
			...
			this_overlap = count_list*1.0/np.minimum(new_all_pixels, areas_of_trias)
			iou = count_list*1.0/(new_all_pixels + areas_of_trias)
			overlap.append(this_overlap)
		elif tria_area != 0:	
			# #### 法一：求两个polygon 交集面积 ####
			# overlap = []
			# for i, triangle in enumerate(trias_all):
			# 	triangle_area = areas_of_trias[i]	
			# 	polygon_overlap = polygon_IOU(tria_2D, triangle, tria_area, triangle_area)
			# 	overlap.append(polygon_overlap)
			...
			for pi in range(x_left, x_right + 1):
				for pj in range(y_top, y_bottom + 1):
					...
					if in_triangle(point, tria_2D):
						...
						count_list[indexes] +=1	
			overlap = count_list *1.0 /np.minimum(tria_area, areas_of_trias)
			iou = count_list *1.0 /(tria_area + areas_of_trias)
	
		inds = np.where(np.array(overlap)>threshold)[0]
		if len(inds):
...
def polygon_IOU(polygon_1, polygon_2, polygon1_area, polygon2_area):
	"""
	计算两个多边形的IOU
	...
	:return:
	"""
	polygon_1 = np.array(polygon_1)
	polygon_2 = np.array(polygon_2)
	rr1, cc1 = skimage.draw.polygon(polygon_2[:, 0], polygon_2[:, 1])
	rr2, cc2 = skimage.draw.polygon(polygon_1[:, 0], polygon_1[:, 1])
	try:
	...
	overlap =  intersection/min(polygon1_area, polygon2_area)
	return overlap

def is_fg_in_3Dspace(info, tria_2D, distance, bbox, tria_area):
	...
	overlap_index = compute_overlap(info, tria_2D, bbox, tria_area)
